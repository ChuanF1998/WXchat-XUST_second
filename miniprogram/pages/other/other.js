Page({
  ...
  onLoad: function (options) {
    ...
    //2、开始查询数据了  news对应的是集合的名称   
    db.collection('second-product').limit(20).orderBy("sell_time", "desc").where({
      sell_shelve: true,// 未下架
      sell_class: "5"
    }).get({
      //如果查询成功的话    
  ...
  onReachBottom: function () {
    var that = this
    if (that.data.tab == 0) {
      let arr1 = that.data.other;
      if (arr1.length < that.data.other_count) {
        const db = wx.cloud.database();
      ...
      if (arr1.length < that.data.other_count) {
        const db = wx.cloud.database();
        db.collection('second-product').skip(arr1.length).limit(5).orderBy("sell_press", "desc").where({
          sell_shelve: true, // 未下架
          sell_class: "3"
        }).get({
          //如果查询成功的话    
